...
import { useCreateClient } from "../../apis/mutation"
...
const CreateClientForm: React.FC<createClientForm> = ({ setOpen }) => {
  ...
  const { handleSubmit, control, watch, setValue, formState: { errors }, } = useForm<clientPayload>({
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        ...
        <div className="space-y-1.5">
          ...
          <Controller
            name="clientName"
            control={control}
            render={({ field }) => <FormInput {...field} placeholder="Client Name" className="w-full" />}
          />
      ...
      <div className="md:col-span-3 flex justify-end gap-3 pt-4">
        ...
        <Button type="submit">{isCreatingClient ? "Creating Client..." : "Create Client"}</Button>
      </div>
    </form>
  )
}
